...
interface ConfiguracionErrorProps {
  error: Error & { digest?: string };
  reset: () => void;
}

export default function ConfiguracionError({ error, reset }: ConfiguracionErrorProps) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-gray-500">
      ...
      <p className="text-sm mb-6 text-gray-400 max-w-md text-center">
        {error.message || "Ocurrió un error al cargar la configuración. Intentá de nuevo."}
      </p>
      <button
        onClick={reset}
