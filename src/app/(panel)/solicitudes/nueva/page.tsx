...
export default function NuevaSolicitudPage() {
  ...
  const [form, setForm] = useState({
    ...
    canal_origen: "telefono" as "telefono" | "whatsapp" | "app" | "presencial",
    asignar_ahora: false,
    unidad_id: "",
  });
      ...
      const sol = await tripService.create({
        ...
        service_type:
          form.tipo_servicio === "carga_pasajeros"
            ? "cargo_passengers"
            : "passengers",
        channel: form.canal_origen === "whatsapp" ? "whatsapp" : form.canal_origen === "app" ? "app" : form.canal_origen === "presencial" ? "presencial" : "phone",
    ...
    <div>
      ...
      <div className="p-8 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Canal de Contacto</h3>
            <div className="flex gap-4">
              {[
                { value: "telefono" as const, label: "Teléfono", icon: "phone" },
                { value: "whatsapp" as const, label: "WhatsApp", icon: "whatsapp" },
                { value: "presencial" as const, label: "Presencial", icon: "person" },
              ].map((opt) => (
                <label
                  key={opt.value}
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 border-2 rounded-lg cursor-pointer transition-colors ${
                    form.canal_origen === opt.value
                ...
                >
                  <input
                    ...
                    name="canal"
                    value={opt.value}
                    checked={form.canal_origen === opt.value}
                    onChange={() => setForm({ ...form, canal_origen: opt.value })}
                    className="sr-only"
                  />
                  {opt.icon === "whatsapp" ? (
            ...
            {form.canal_origen === "whatsapp" && form.telefono_pasajero && (
              <p className="text-xs text-green-600 flex items-center gap-1">
                ...
                Se abrirá WhatsApp al número {form.telefono_pasajero}
              </p>
            )}
          </div>
